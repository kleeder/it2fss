...
for use with fsound.exe. Effects commands are still not supported.
Kick and Snare Samples are supported!

You have to use the Volume Column for every new note. Volumes 0 to 15 are mapped
...
def note_format(note, rows, vol, cmdVal, tempo):
    if vol != None:
        if vol == 10:
            vol = "a"
        elif vol == 11:
...
def convert(module, filename):
    ...
    outfile.write('> generated by it2fss.py Ver 0.3\n\n')
    ...
    for order in (x for x in module.orders if x != 255):
        ...
        for row in range(len(pattern[0])):
            ...
            if cur_item is not None and cur_item != item or cur_item is not None and cur_vol != vol:
                outfile.write(note_format(item, length, vol, cur_cmdVal, module.tempo))
                length = 0
                item = cur_item
                vol = cur_vol
            length += 1
